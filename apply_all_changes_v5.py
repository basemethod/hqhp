...
def process_file(file_path):
    ...
    # 2. Fix ONLY the recruit block in index.html
    if file_path.endswith('index.html') and 'tohoku_2025/index.html' in file_path:
        content = re.sub(
            r'(<div id="recruit_block".*?<div class="images">.*?<ul class="flex">).*?(</ul>)',
    ...
    # 3. Fix ONLY the sponsorList in all files
    ...
    if sponsor_match:
        ...
        sponsor_inner = re.sub(
            r'<li>\s*<img[^>]*wp-content/uploads/new_assets/fukaya\.jpg[^>]*>\s*</li>',
            '<li><div style="border: 3px solid white; color: white; padding: 10px; text-align: center; min-width: 100px;">外部リンク1</div></li>',
            sponsor_inner
        )
        sponsor_inner = re.sub(
            r'<li>\s*<img[^>]*wp-content/uploads/new_assets/shizukanagogo\.jpg[^>]*>\s*</li>',
            '<li><div style="border: 3px solid white; color: white; padding: 10px; text-align: center; min-width: 100px;">外部リンク2</div></li>',
            sponsor_inner
        )
        sponsor_inner = re.sub(
    ...
    # 4. Remove external links elsewhere but keep the text/img
